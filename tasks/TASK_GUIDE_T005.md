...
|-------|--------|------------------------|
| **New test(s) cover Acceptance Criteria (file paths pasted)** | ✅ N/A | Documentation-only task — verification is manual per Success Criteria above, all commands re-verified live and pass |
| Verification command run | ✅ pass | Every command/code block in both docs run verbatim against a freshly rebuilt container: AGENTS.md's 4-step MCP flow, the initialize curl example, HARNESS.md's /api/run-test curl example, and the bash script example (run as an actual #!/bin/bash script, not pasted inline) — all match documented output exactly |
| Negative cases hold | ✅ N/A | Documentation task — no negative-path code to test |
| verify | ✅ pass | Full step-by-step walkthrough of both docs against a live rebuilt container; all commands pass. Found and fixed during code review: a factual inaccuracy (claimed src/tools/mobile.ts stub that doesn't exist), an incorrect Claude Code MCP config shape, and a protocol-version inconsistency — all corrected and re-verified live. See verification report in conversation, 2026-07-01. |
| Review scope bounded to the change's blast radius | ✅ pass | Reviewed only the two new files: `AGENTS.md`, `HARNESS.md` — no code changes in this task |
| Full smoke suite still green (no regression) | ✅ N/A | Documentation-only task, no code touched, no test suite affected |
