...
### 2026-08-17 — Logic inside a per-connection closure is invisible to the test suite (T013 review)
**Pattern**: `recordAction` lives inside `buildMcpServer()` in `src/server.ts` and is never exported, so nothing in `test/` can reach it. T013's soft-check tests all hand-built a `LoggedAction` with `soft: true` already set — meaning the one line that *decided* `soft` from an `assertCondition` result was untested, and inverting it would have left all 19 tests green. Fixed by extracting `classifyCheckOutcome()` into `web.ts` as a pure function. Watch for this shape generally: when tests construct the very value the production code is supposed to compute, they are asserting the fixture, not the behaviour. Ask "which line would I have to break for this test to fail?"
**Files**: `src/server.ts`, `src/tools/web.ts`, `test/session.test.ts`
